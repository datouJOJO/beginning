...
public class PhotoServiceImpl extends ServiceImpl<PhotoDao, Photo> implements PhotoService {
    ...
    @Override
    public PhotoDTO listPhotoById(Integer albumId) {
        PhotoAlbums albums = photoAlbumsDao.selectOne(new LambdaQueryWrapper<PhotoAlbums>()
                .eq(PhotoAlbums::getId, albumId)
        ...
        return PhotoDTO.builder()
                .photoAlbumCover(albums.getAlbumCover())
                .photoAlbumName(albums.getAlbumName())
                .photoList(photoList)
                .build();
    }
...
// 更新照片状态
        ...
        if (deleteVO.getIsDelete().equals(FALSE)) {
            List<PhotoAlbums> photoAlbumList = photoDao.selectList(new LambdaQueryWrapper<Photo>()
                    .select(Photo::getAlbumId)
                    .in(Photo::getId, deleteVO.getIdList())
                    .groupBy(Photo::getAlbumId))
                    .stream()
                    .map(item -> PhotoAlbums.builder()
                            .id(item.getAlbumId())
                            .isDelete(FALSE)
                            .build())
                    .collect(Collectors.toList());
            photoAlbumsService.updateBatchById(photoAlbumList);
        }
    }
